...
A [SPOE (Stream Processing Offload Engine)](https://www.haproxy.com/blog/extending-haproxy-with-the-stream-processing-offload-engine) agent for HAProxy 2.8 that adds distributed tracing via OpenTelemetry. It creates a server-side span for every HTTP request/response pair and exports them over gRPC OTLP to Tempo or any compatible backend.

HAProxy 3.2 will ship native OpenTelemetry support. This agent bridges the gap for deployments running HAProxy 2.8.
...
**Prerequisites:** Go 1.25 or later, `make`.
